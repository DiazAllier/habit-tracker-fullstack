...
const SRAButton = ({ userId, workoutId, session, onSessionUpdate, onLongRun, onAddExercise }) => {
  ...
  const handleStopWorkout = async () => {
    ...
    const elapsedSeconds = Math.max(0, Math.floor((Date.now() - new Date(session.startedAt)) / 1000));
    const caloriesBurned = calculateSessionCalories(session, elapsedSeconds);

    try {
      await completeWorkout(session.id, caloriesBurned);
